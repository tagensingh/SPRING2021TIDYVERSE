...
Because we built this into a `workflow()`, we can just change the model part of things and run the pipeline easily. 
...
pet_pred %>% 
  roc_auc(truth = AdoptedFast, .pred_no)
```
That's barely any better than the first model.  One other model that usually gets thrown at this type of ML is a random forest.  It handles the categorical data well, and deals with overfitting by not letting the model rely too much on any one feature, which probably won't get used in most of the trees the model builds.  Again we can just insert it into the workflow and give it a whirl.  
...
pet_pred %>% 
  ...
  autoplot()
```
That performs better.  The other ones had an ROC-AUC score of about .66, so let's see how much better the random forest did.  
